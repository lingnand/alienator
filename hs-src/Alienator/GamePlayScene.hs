...
buildDiffs
  :: forall t host m a b. NodeBuilder t host m
  => (P.Id -> DynStateT t a m b) -> DynStateT t (P.Pool a) m (Event t [b])
buildDiffs onDiff = do
    ...
    forEvent diffE . P.traverseWithId $ \pid ps -> do
      focus (at pid . pnon ps) $ onDiff pid :: DynStateT t (P.Pool a) m b
...
gamePlayScene
  :: forall t host m. (NodeBuilder t host m)
  ...
  -> DynStateT t (GamePlaySceneState host) m (Event t ()) -- ^ return when game is over
gamePlayScene winSize sp steps collisionsE keysDyn = seqHoldFree buildFree
  where buildFree :: FreeT (Event t) (DynStateT t (GamePlaySceneState host) m) ()
        buildFree = do
          ...
          playerDyn' <- lift . focus playerShip $ do
            playerHits <- focus pSprite $ do
              ...
              forEvent_ hits $ \ct -> putStrLn $ "Player got hit with " ++ show ct
          ...
          lift $ do
            ...
            bulletPoolModEE <- focus enemyShipPool $ do
              let -- randomly add movement into the enemy
                  maybeReactuateModifier :: R.Rand R.StdGen (Maybe (VelActuator -> VelActuator))
                  ...
                  maybeAttackModifier :: Int -> VelActuator -> R.Rand R.StdGen (Maybe [BulletState host])
              ...
              enemyModsE <- buildDiffs $ \pid -> do
                ...
                bulletsE <- seqEventMaybe $ attachWith (const . maybeAttackModifier bulletT) actBeh $ gate enabledBeh fps1
              ...
              let (resetModEE, bulletPoolModEE) = splitE $ ffor enemyModsE $ \mods ->
                                                      let (as, bs) = unzip mods
                                                          f = mergeWith (.)
                                                      in (f as, f bs)
              seqEvent_ $ (adjust <$> resetModEE :: Event t (DynStateT t (P.Pool (PSpriteState' VelActuator)) m ()))

              -- generate enemies
              randSpawnEnemiesE <- forEvent fps5 . const $ do
                  ...
                  return $ (actuator  .~ (def & pos .~ 0 .+^ rv2 & modifier))
                         ...
                         . (enabled   .~ True) :: R.Rand _ (PSpriteState' VelActuator -> PSpriteState' VelActuator)
            ...
            focus bulletPool $ do
              ...
              seqEvent_ $ (adjust . mergeWith (.) <$> resetModsE :: Event t (DynStateT t (P.Pool (BulletState host)) m ()))
              ...
              seqEvent_ $ (adjust <$> bulletPoolModEE :: Event t (DynStateT t (P.Pool (BulletState host)) m ()))
